...
from markets import SimplifiedDoubleAuctionMarket
...
class Experiment():
    ...
    def __init__(self, true_value: float,
        ...
        such as SimplifiedDoubleAuctionMarket
    ...
    def run_experiment(self):
        ...
        """
        self.market.run_market(self.number_of_trading_rounds)
        ...
        try:
            ...
            pass
        combined_prices = self.market.yes_prices + [1 - x for x in self.market.no_prices]
        if len(combined_prices) > 0:
            res = np.mean(combined_prices)
        else:
            res = 0.5
        if self.mode == 'binary':
...
def run_experiment_series(list_of_participant_numbers, config, n=10, true_value=None, pright=None):
    """
    config should be a dictionary with the following keys:
    mode: either 'binary' or 'index'
    market_class: the class of market to be used (e.g. SimplifiedDoubleAuctionMarket or MarketMakerMarket)
    pright: probability that a participant is correct in binary markets
    take_market_uncertainty_into_account: whether to take market uncertainty into account when deciding whether to trade
    """

    try:
        take_market_uncertainty_into_account = config['take_market_uncertainty_into_account']
    except KeyError:
        take_market_uncertainty_into_account = config['mode'] == 'index'

    try:
        pright = config['pright']
    except KeyError:
        if config['mode'] == 'binary':
            raise ValueError('No pright was specified in config.')
